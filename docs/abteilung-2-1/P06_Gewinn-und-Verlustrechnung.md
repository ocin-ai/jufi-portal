# Prozess 06: Gewinn- und Verlustrechnung

> Abteilung 2.1 Wareneinkauf / Lager · Status: überarbeitet · zuletzt bearbeitet: 2026-07-07

## Steckbrief

| | |
| --- | --- |
| **Prozess-Nr.** | 6 (Übersicht: „Umsatzauswertung / GuV") |
| **Verantwortlich** | → [Übersicht](00_Uebersicht-Prozesse-Verantwortlichkeiten.md) |
| **Turnus** | regelmäßig, auf Monatsbasis |
| **Schnittstellen** | Team Digitalisierung Einkaufsbelege |

## Ziel / Kurzbeschreibung

Ziel ist es, eine Orientierungsgröße für den Beitrag einzelner Artikel zum Gewinn zu ermitteln.

## Relevante Dateien

| Datei | Dateiname | Dateipfad |
| --- | --- | --- |
| Gewinn- und Verlustrechnung (Excel) | 25-26_06_Gewinn-und-Verlustrechnung | Ablage_2.1_Wareneinkauf-Lager/06_Gewinn-und-Verlustrechnung/ |
| Umsatzauswertung / SumUp-Export | vgl. [P05](P05_Umsatzauswertung-SumUp.md) | vgl. P05 |
| Einkaufsbelege (digital) | vgl. [P04](P04_Einkaufsbelege-digitalisieren.md) | vgl. P04 |
| Konzept-Notiz (Aufbau der Excel; Datei folgt bei Bedarf) | 25-26_06_Gewinn-und-Verlustrechnung_Konzept.md | Ablage_2.1_Wareneinkauf-Lager/06_Gewinn-und-Verlustrechnung/ |

## Prozessschritte

- [ ] Öffnen der Datei `25-26_06_Gewinn-und-Verlustrechnung`.
- [ ] Eingabe der aktuellen Verkaufspreise der einzelnen Artikel.
- [ ] Umsatzauswertung aus SumUp herunterladen (vgl. [P05](P05_Umsatzauswertung-SumUp.md)).
- [ ] Umwandlung der .csv-Datei in eine Excel-Tabelle mit Datenfilter.
- [ ] Selektion der relevanten Informationen (Preis, umgesetzte Menge).
- [ ] Gegenüberstellung Verkaufspreis und durchschnittlicher Einkaufspreis (Tabelle: digitale Einkaufsbelege, vgl. [P04](P04_Einkaufsbelege-digitalisieren.md)).
- [ ] Beim Aufwand sind die Fahrtkosten zu berücksichtigen. Hierzu ist ggf. ein pauschaler Ansatz zu wählen. Wie dieser ermittelt wird, ist noch festzulegen.
- [ ] Ermittlung Gewinn pro Stück.
- [ ] Recherche (Archivordner), wie dies im Vorjahr durchgeführt wurde.

## Offene Punkte / tbd.

- ~~Methode Fahrtkostenpauschale~~ – entschieden 2026-07-17: Prozent-Zuschlag wie [P14](P14_Preiskalkulation.md) (Parameterzelle 10 % in der GuV-Excel). Hintergrund: Die real gezahlte Kilometerpauschale beträgt 1,00 €/km (Fahrer-Regelung, Verwaltung über Abt. 1 P11) – der Zuschlagssatz kann daran später kalibriert werden.
