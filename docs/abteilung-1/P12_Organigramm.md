# Prozess 12: Organigramm erstellen / pflegen

> Abteilung 1 Sekretariat / Personal · Status: Entwurf · zuletzt bearbeitet: 2026-07-17

## Steckbrief

| | |
| --- | --- |
| **Prozess-Nr.** | 12 |
| **Verantwortlich** | → [Übersicht](00_Uebersicht-Prozesse-Verantwortlichkeiten.md) |
| **Turnus** | Schuljahresbeginn + bei Änderungen [tbd.] |
| **Schnittstellen** | Geschäftsleitung · alle Abteilungen |

## Ziel / Kurzbeschreibung

Erstellung und Pflege des Jufi-Organigramms (Abteilungen 1–4 mit Unterbereichen, Zuordnung der Verantwortlichen).

## Relevante Dateien

| Datei | Dateiname | Dateipfad |
| --- | --- | --- |
| Organigramm (PowerPoint, WES-Design; Folie 1 intern mit Kürzeln, Folie 2 ohne Kürzel) | 2526_12_Organigramm_v2 | 1_Sekretariat-Personal/12_Organigramm/ |
| Organigramm für Veröffentlichung/Portal (PNG, ohne Kürzel) | 2526_12_Organigramm_Portal_v2 | 1_Sekretariat-Personal/12_Organigramm/ |
| Vorversion (PowerPoint) | 2526_12_Organigramm_v1 | 1_Sekretariat-Personal/12_Organigramm/ |
| Vorjahres-Vorlage (Word, Archiv) | – | `99_Archiv/Organigramm/2425_BK2J_Jufi_Organigramm_S_v1.docx` (Lehrkraftbereich) |

## Prozessschritte

- [ ] Zu Schuljahresbeginn: Organigramm aus dem Vorjahr übernehmen und aktualisieren (Abteilungen, Kürzel der Verantwortlichen).
- [ ] Bei Änderungen im Schuljahr: Organigramm nachführen, neue Version speichern (`_vX`).
- [ ] Veröffentlichung/Aushang. [tbd. – wo?]

## Offene Punkte / tbd.

- Veröffentlichung/Aushang klären (siehe Prozessschritte); fürs Portal NUR die kürzelfreie Fassung verwenden.
- ~~Prüfpunkte aus Sichtung 2026-07-07~~ – alle entschieden 2026-07-17:
  - Doppeltes Diagramm: in v1/v2 bereits bereinigt (eine Folie je Fassung).
  - Verortung 4.2 Snackautomat/FM: bleibt unter 4 Verwaltung.
  - Kasteninhalte: bleiben schlicht (nur Name + Kürzel); Aufgaben stehen in den Prozessübersichten.
